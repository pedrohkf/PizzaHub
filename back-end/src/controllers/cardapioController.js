...
exports.criarCardapio = async (req, res) => {
    try {
        const { pizzariaId, categorias } = req.body;

        const cardapioExistente = await Cardapio.findOne({ pizzariaId });
        if (cardapioExistente) {
            return res.status(400).json({ message: "Esta pizzaria já possui um cardápio." });
        }

        const novoCardapio = new Cardapio({ pizzariaId, categorias });
...
exports.obterCardapio = async (req, res) => {
    try {
        ...
        const cardapio = await Cardapio.findOne({ pizzariaId });
